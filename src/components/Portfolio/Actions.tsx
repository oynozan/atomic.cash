...
export default function PortfolioActions() {
  ...
  const tokenReceiveAddress = useMemo(() => {
    ...
    try {
      ...
      const tokenType =
        decoded.type === "p2sh" ? "p2shWithTokens" : "p2pkhWithTokens";
    ...
    try {
      const res = await fetch("/api/tx/send", {
        ...
        body: JSON.stringify({
          ...
          amount: value,
          tokenCategory:
            sendAssetType === "token" && selectedToken ? selectedToken.category : undefined,
        }),
      });
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!selectedToken && selectableTokens[0]) {
                        setSelectedTokenCategory(selectableTokens[0].category);
                      }
                    }}
                    className={`px-3 py-1.5 rounded-full font-semibold transition-colors ${
                      sendAssetType === "token"
                      ...
                      <DropdownMenu>
                        ...
                        >
                          {selectableTokens.map((t) => (
                            <DropdownMenuItem
                              key={t.category}
                              onSelect={() => setSelectedTokenCategory(t.category)}
