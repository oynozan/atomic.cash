    ...
    try {
        ...
        for (const tx of txs) {
            ...
            if (tx.direction === "bch_to_token") {
                ...
                const tokenOut = typeof a.tokenOut === "number" && a.tokenOut > 0 ? a.tokenOut : 0;
                bch += bchIn;
                ...
                const bchOut = typeof a.bchOut === "number" && a.bchOut > 0 ? a.bchOut : 0;
                const tokenIn = typeof a.tokenIn === "number" && a.tokenIn > 0 ? a.tokenIn : 0;
                bch -= bchOut;
                const prev = tokens.get(tx.tokenCategory) ?? 0;
        ...
        const swappedThisWeekBch = txsThisWeek.reduce((sum, tx) => sum + extractBchVolume(tx), 0);
