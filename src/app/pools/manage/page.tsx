  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-muted-foreground">
        Add liquidity to your micro pool. BCH and token amounts will follow the current pool ratio.
      </p>
      ...
      <div className="flex gap-2 rounded-full border bg-background/40 p-1 text-xs font-medium">
        <button
          ...
          className={`flex-1 rounded-full px-3 py-1 ${
            addMode === "bch" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
          }`}
          onClick={() => {
            setAddMode("bch");
            setAddAmount("");
        ...
        <button
          type="button"
          className={`flex-1 rounded-full px-3 py-1 ${
            addMode === "token" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
          }`}
          onClick={() => {
            setAddMode("token");
            setAddAmount("");
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/40 bg-emerald-500/5 px-3 py-1 text-xs font-medium text-emerald-200">
          ...
          <span>
            ...
            <span className="font-mono">
              {formatNumber(previewBch, 8)} BCH + {formatNumber(previewToken, 4)} {tokenLabel}
            </span>
          </span>
        </div>
      )}
...
function RemoveLiquidityContent({
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-muted-foreground">
        Remove liquidity proportionally from BCH and token reserves. Pool price will stay the same.
      </p>

      <div className="space-y-3">
        <label className="block text-sm font-medium leading-none mb-2">Percentage to withdraw</label>
        <div className="flex items-center justify-between gap-3 rounded-[20px] border bg-popover px-4 py-3">
          <input
            type="number"
            min="1"
...
export default function ManagePoolsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/user/pools?address=${encodeURIComponent(address)}`)
      ...
      .then((json: UserPoolsResponse) => {
        if (!cancelled) setData(json);
      })
      .catch((err) => {
        if (!cancelled) setError(err instanceof Error ? err.message : "Failed to load your pools");
      })
      .finally(() => {
        if (!cancelled) {
          // no-op
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center -space-x-3">
                      <TokenAvatar
                        symbol={group.tokenSymbol ?? group.tokenCategory.slice(0, 8) + "…"}
                    ...
                    <div className="flex flex-col">
                      <span className="text-lg font-semibold">
                        {group.tokenSymbol ?? group.tokenCategory.slice(0, 8) + "…"} - BCH
                      </span>
                      ...
                      <span className="text-[11px] text-muted-foreground">
                        ...
                        <span className="font-mono text-foreground">
                          ...
                          {group.tokenSymbol ?? group.tokenCategory.slice(0, 4) + "…"}
                        </span>
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  {group.pools
                    ...
                    .map((pool, idx) => (
                      ...
                      >
                        <div className="flex flex-col">
                          <span className="font-medium">Micro pool #{idx + 1}</span>
                          <span className="text-[11px] text-muted-foreground">
                            Pool address: {pool.poolAddress.slice(0, 10)}…
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              className="text-xs"
                              onClick={() => {
                                setActiveModal({ type: "remove", pool });
                                setRemovePercentage("");
                                setRemoveAll(false);
                              }}
                            >
