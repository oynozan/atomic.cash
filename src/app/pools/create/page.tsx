...
function TokenIcon({ iconUrl, symbol }: { iconUrl?: string | null; symbol: string }) {
    if (iconUrl) {
        return (
            <img src={iconUrl} alt={symbol} className="size-6 rounded-full object-cover shrink-0" />
        );
    }
    return (
...
export default function CreatePoolPage() {
    ...
    // Fetch balances once per address (reuses shared store with TTL + in-flight dedupe)
    useEffect(() => {
        if (!address?.trim()) return;
        let cancelled = false;
        setBalancesLoading(true);
        fetchBalances(address).finally(() => {
    ...
    useEffect(() => {
        ...
        const tokens: TokenItem[] = balances.tokens
            .filter(t => t.amount > 0)
            .map(t => ({
                category: t.category,
                symbol: t.symbol,
                ...
                amount: t.amount,
                amountRaw: t.amountRaw ?? String(t.amount),
                iconUrl: t.iconUrl,
            }));
        if (tokens.length > 0) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (hasMarketPools && marketPrice && marketPrice > 0) {
            ...
            if (
                ...
                toast.error(
                    "For the first pool, provide both BCH and token amounts (positive numbers).",
                );
                return;
            }
            bch = parsedBch;
    ...
    const tokensWithBalance: TokenItem[] =
        balances?.tokens
            ?.filter(t => t.amount > 0)
            .map(t => ({
                category: t.category,
        ...
        <section className="w-full min-w-0 pt-28 sm:pt-36 lg:pt-44 pb-24 sm:pb-32 flex justify-center">
            <div className="home-container max-w-xl">
                ...
                <form
                    ...
                    className="space-y-6 rounded-[24px] border bg-secondary/80 p-6"
                >
                                                ...
                                                <>
                                                    <TokenIcon
                                                        ...
                                                        symbol={
                                                            ...
                                                            selectedToken.category.slice(0, 8)
                                                        }
                                                    />
                                                    <span>
                                                        {selectedToken.symbol ??
